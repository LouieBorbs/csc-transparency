var API = {
  ...
  insert: function(table, row) {
    ...
    }).then(function(r) { return r.json(); }).then(function(r) { return r.data; });
  },
  update: function(table, id, changes) {
    return fetch('/api/data/' + table + '/' + id, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  remove: function(table, id) {
    return fetch('/api/data/' + table + '/' + id, { method: 'DELETE' })
      .then(function(r) { return r.json(); });
  },
  getFinance: function() {
    return fetch('/api/data/finance').then(function(r) { return r.json(); }).then(function(r) { return r.data; });
  },
  updateFinance: function(data) {
    return fetch('/api/data/finance', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
loadData: async function() {
        var self = this;
        var tables = [
            'users','events','announcements','files','polls','suggestions',
        ...
        var results = await Promise.all(tables.map(function(t) { return API.get(t); }));
        tables.forEach(function(t, i) { self.data[t] = results[i]; });
        self.data.finance = await API.getFinance() || { currentFunds: 50000, transactions: [] };
        var pmRes = await fetch('/api/data/positionMappings').then(function(r) { return r.json(); });
        self.data.positionMappings = pmRes.data || {};
        self.data.otpCodes = {};
        self.data.attendanceRecords = self.data.attendanceRecords || [];
    },

    saveData: function() {
        // Data is now saved directly to Supabase via API calls
        // Individual save calls happen at each action
    },
