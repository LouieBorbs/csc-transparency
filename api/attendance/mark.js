...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      let sessionId = null, eventName = 'Unknown Event', expiresAt = null;
      try {
      ...
      if (!sessionId) return res.status(400).json({ success: false, message: 'Invalid QR code' });
      if (expiresAt && new Date(expiresAt) < new Date()) return res.status(400).json({ success: false, message: 'QR code expired' });
      ...
      const { data, error } = await supabase.from('attendance_records').insert({
        ...
        event_name: eventName, status: 'present',
