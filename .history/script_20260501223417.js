        ...
        if (qrCodesList.length === 0) {
            ...
            qrCodesList.forEach(function(q) {
                ...
                html += '<tr><td>' + q.name + '</td><td>' + (q.eventTitle || '-') + '</td><td><code style="font-size:11px;">' + q.code + '</code></td><td>' + q.date + '</td><td><span class="badge badge-info">' + attendeeCount + '</span></td>' +
                 ...
                 '<li><button class="dropdown-item" data-action="view-qr-code" data-id="' + q.id + '" data-url="' + encodeURIComponent(qrUrl) + '"><i class="fas fa-qrcode"></i> View QR</button></li>' +
                 '<li><button class="dropdown-item" data-action="view-qr-attendees" data-id="' + q.id + '"><i class="fas fa-users"></i> View Attendees</button></li>' +
                 (attendeeCount > 0 ? '<li><button class="dropdown-item" data-action="export-qr-attendance" data-id="' + q.id + '"><i class="fas fa-file-excel"></i> Export Excel</button></li>' : '') +
                 '<li><button class="dropdown-item" data-action="edit-qr-code" data-id="' + q.id + '"><i class="fas fa-edit"></i> Edit</button></li>' +
                 '<li><button class="dropdown-item" data-action="delete-qr-code" data-id="' + q.id + '"><i class="fas fa-trash"></i> Delete</button></li>' +
                 '</ul>' +
                 '</div></td></tr>';
            });
        ...
        if (qrGeneratorForm) {
            qrGeneratorForm.addEventListener('submit', function(e) {
                ...
                if (displayModal && displayTitle && displayImageContainer && displayInfo) {
                 ...
                 // Auto-download the QR code with timeout to ensure it works
                 setTimeout(function() {
                     var link = document.createElement('a');
                     link.href = qrUrl;
