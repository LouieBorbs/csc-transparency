    ...
    loadData: function() {
        try {
            ...
            if (stored) {
                ...
                if (!this.data.postRequests) this.data.postRequests = [];
                if (!this.data.qrCodes) this.data.qrCodes = [];
                if (!this.data.messages) this.data.messages = [];
                if (!this.data.comments) this.data.comments = [];
                if (!this.data.organizations) this.data.organizations = [
                    { id: 1, name: 'College Student Council', committees: ['Executive', 'Finance', 'Public Relations', 'Secretariate', 'Documentation', 'Events'] },
                    { id: 2, name: 'IT Club', committees: ['Executive', 'Technical', 'Secretariate', 'Documentation', 'Public Relations'] },
                    { id: 3, name: 'HM Club', committees: ['Executive', 'Hospitality', 'Secretariate', 'Documentation', 'Public Relations'] },
                    { id: 4, name: 'CPE Club', committees: ['Executive', 'Technical', 'Secretariate', 'Documentation', 'Public Relations'] },
                    { id: 5, name: 'BA Club', committees: ['Executive', 'Finance', 'Secretariate', 'Documentation', 'Public Relations'] },
                    { id: 6, name: 'BSAIS Club', committees: ['Executive', 'Finance', 'Secretariate', 'Documentation', 'Public Relations'] },
                    { id: 7, name: 'Junior Council', committees: ['Executive', 'Student Affairs', 'Events'] }
                ];
                if (!this.data.positionMappings) this.data.positionMappings = {
...
renderAdminContent: function() {
    ...
    renderAdminAttendance: function() {
        ...
        html += '<div id="qr-generator-modal" class="modal" style="display:none;align-items:center;justify-content:center;">' +
            '<div class="modal-content" style="max-width:500px;">' +
            '<div class="modal-header"><h3>Generate Attendance QR Code</h3><button class="modal-close" id="close-qr-generator-modal">&times;</button></div>' +
        ...
        html += '</select></div>' +
            ...
            '<input type="date" class="form-input" id="qr-date-input" required></div>' +
            '</div>' +
            '<div class="modal-actions"><button type="submit" class="btn btn-primary">Generate & Download QR</button></div>' +
            '</form></div></div>';
        
        // QR Code Display Modal (Popup - No minimize, only X close)
        html += '<div id="qr-display-modal" class="modal" style="display:none;align-items:center;justify-content:center;z-index:10000;">' +
            '<div class="modal-content" style="max-width:400px;text-align:center;position:relative;">' +
            '<div class="modal-header">' +
            '<h3 id="qr-display-title">QR Code Generated</h3>' +
            '<button class="modal-close" id="close-qr-display-modal" style="cursor:pointer;">&times;</button>' +
            '</div>' +
            '<div style="padding:24px;">' +
            '<div id="qr-display-image-container" style="margin-bottom:16px;"></div>' +
            '<div id="qr-display-info" style="font-size:13px;color:var(--text-light);margin-bottom:16px;"></div>' +
            '<p style="font-size:12px;color:var(--text-light);">QR Code has been automatically downloaded to your device.</p>' +
            '</div></div></div>';
        ...
        if (qrCodesList.length === 0) {
            ...
            html += '<div class="table-container"><table class="table"><thead><tr><th>Name</th><th>Event</th><th>Code</th><th>Date</th><th>Attendees</th><th style="min-width:280px;">Actions</th></tr></thead><tbody>';
            qrCodesList.forEach(function(q) {
                ...
                html += '<tr><td>' + q.name + '</td><td>' + (q.eventTitle || '-') + '</td><td><code style="font-size:11px;">' + q.code + '</code></td><td>' + q.date + '</td><td><span class="badge badge-info">' + attendeeCount + '</span></td>' +
                '<td><div style="display:flex;gap:4px;flex-wrap:wrap;">' +
                '<button class="btn btn-sm btn-warning" data-action="view-qr-code" data-id="' + q.id + '" data-url="' + encodeURIComponent(qrUrl) + '" title="View QR"><i class="fas fa-qrcode"></i></button>' +
                '<button class="btn btn-sm btn-info" data-action="view-qr-attendees" data-id="' + q.id + '" title="View Attendees"><i class="fas fa-users"></i></button>' +
                '<button class="btn btn-sm btn-secondary" data-action="edit-qr-code" data-id="' + q.id + '" title="Edit"><i class="fas fa-edit"></i></button>' +
                '<button class="btn btn-sm btn-danger" data-action="delete-qr-code" data-id="' + q.id + '" title="Delete"><i class="fas fa-trash"></i></button>' +
                (attendeeCount > 0 ? '<button class="btn btn-sm btn-primary" data-action="export-qr-attendance" data-id="' + q.id + '" title="Export Excel"><i class="fas fa-file-excel"></i></button>' : '') +
                '</div></td></tr>';
            });
        ...
        // QR Attendees Modal (Attendees Detail Table)
        html += '<div id="qr-attendees-modal" class="modal" style="display:none;align-items:center;justify-content:center;">' +
            ...
            '<div class="modal-header"><h3 id="qr-attendees-title">Attendees Detail</h3><button class="modal-close" id="close-qr-attendees-modal">&times;</button></div>' +
            '<div style="padding:16px;">' +
            '<div id="qr-attendees-count" style="margin-bottom:12px;font-weight:600;"></div>' +
            '<button class="btn btn-primary" id="export-qr-attendees-btn" style="margin-bottom:12px;"><i class="fas fa-file-excel"></i> Export to Excel</button>' +
            '<div class="table-container" style="max-height:400px;overflow:auto;">' +
            '<table class="table" id="attendees-detail-table"><thead><tr><th>Full Name</th><th>Email</th><th>Course</th><th>Section</th><th>Date Scanned</th><th>Time Scanned</th></tr></thead><tbody id="attendees-tbody"></tbody></table></div>' +
            '</div></div></div>';
        
        // Edit QR Code Modal
        html += '<div id="qr-edit-modal" class="modal" style="display:none;align-items:center;justify-content:center;">' +
            '<div class="modal-content" style="max-width:500px;">' +
            '<div class="modal-header"><h3>Edit QR Code</h3><button class="modal-close" id="close-qr-edit-modal">&times;</button></div>' +
            '<form id="qr-edit-form">' +
            '<input type="hidden" id="qr-edit-id">' +
            '<div style="padding:20px;">' +
            '<div class="form-group"><label class="form-label">QR Code Name *</label>' +
            '<input type="text" class="form-input" id="qr-edit-name" required></div>' +
            '<div class="form-group"><label class="form-label">Event</label>' +
            '<select class="form-input" id="qr-edit-event">' +
            '<option value="">Select Event (or leave blank)</option>';
        ...
        html += '</select></div>' +
            '<div class="form-group"><label class="form-label">Date *</label>' +
            '<input type="date" class="form-input" id="qr-edit-date" required></div>' +
            '</div>' +
...
'<button type="submit" class="btn btn-primary btn-block" style="margin-top:16px;">Save Changes</button>' +
        ...
        // Generate QR Code Button - Open Modal
        ...
        if (generateQrBtn) {
            generateQrBtn.addEventListener('click', function() {
                ...
                if (modal) {
                    ...
                    var dateInput = document.getElementById('qr-date-input');
        ...
        var closeQrGeneratorModal = document.getElementById('close-qr-generator-modal');
        if (closeQrGeneratorModal) {
            closeQrGeneratorModal.addEventListener('click', function() {
                var modal = document.getElementById('qr-generator-modal');
                if (modal) modal.style.display = 'none';
            });
        }
        
        // QR Generator Form Submit
        var qrGeneratorForm = document.getElementById('qr-generator-form');
        if (qrGeneratorForm) {
            qrGeneratorForm.addEventListener('submit', function(e) {
                ...
                var qrDate = document.getElementById('qr-date-input').value;
                ...
                self.data.qrCodes.push({
                    ...
                    date: qrDate || new Date().toISOString().split('T')[0],
                    attendees: []
                });
                self.saveData();
                
                // Close generator modal
                var genModal = document.getElementById('qr-generator-modal');
                if (genModal) genModal.style.display = 'none';
                
                // Show QR Display Modal with the generated QR code
                var displayModal = document.getElementById('qr-display-modal');
                var displayTitle = document.getElementById('qr-display-title');
                var displayImageContainer = document.getElementById('qr-display-image-container');
                var displayInfo = document.getElementById('qr-display-info');
                
                if (displayModal && displayTitle && displayImageContainer && displayInfo) {
                    displayTitle.textContent = 'QR Code Generated: ' + codeName;
                    displayImageContainer.innerHTML = '<img src="' + qrUrl + '" style="width:250px;height:250px;border-radius:8px;">';
                    displayInfo.innerHTML = '<strong>Code:</strong> ' + code + '<br><strong>Event:</strong> ' + eventTitle;
        ...
        // Close QR Display Modal (only X button - no minimize)
        var closeQrDisplayModal = document.getElementById('close-qr-display-modal');
        if (closeQrDisplayModal) {
            closeQrDisplayModal.addEventListener('click', function() {
                var modal = document.getElementById('qr-display-modal');
                if (modal) modal.style.display = 'none';
            });
        }
        
        // Edit QR Code Button
        document.querySelectorAll('[data-action="edit-qr-code"]').forEach(function(btn) {
            ...
            btn.addEventListener('click', function() {
                ...
                if (qr) {
                    var modal = document.getElementById('qr-edit-modal');
                    var closeBtn = document.getElementById('close-qr-edit-modal');
                    var form = document.getElementById('qr-edit-form');
                    
                    if (modal && closeBtn && form) {
                        document.getElementById('qr-edit-id').value = qr.id;
                        document.getElementById('qr-edit-name').value = qr.name;
                        document.getElementById('qr-edit-date').value = qr.date;
                        if (qr.eventId) {
                            document.getElementById('qr-edit-event').value = qr.eventId;
                        }
                        modal.style.display = 'flex';
                    }
                }
        ...
        // QR Edit Form Submit
        var qrEditForm = document.getElementById('qr-edit-form');
        if (qrEditForm) {
            qrEditForm.addEventListener('submit', function(e) {
                e.preventDefault();
                var editId = parseInt(document.getElementById('qr-edit-id').value);
                var qr = self.data.qrCodes.find(function(q) { return q.id === editId; });
                if (qr) {
                    qr.name = document.getElementById('qr-edit-name').value;
                    qr.date = document.getElementById('qr-edit-date').value;
                    var eventId = document.getElementById('qr-edit-event').value;
                    qr.eventId = eventId || null;
                    qr.eventTitle = eventId ? (self.data.events.find(function(e) { return e.id == eventId; }) || {}).title : qr.name;
                    self.saveData();
                    document.getElementById('qr-edit-modal').style.display = 'none';
                    self.renderAdminDashboard();
        ...
        // Close QR Edit Modal
        var closeQrEditModal = document.getElementById('close-qr-edit-modal');
        if (closeQrEditModal) {
            closeQrEditModal.addEventListener('click', function() {
                var modal = document.getElementById('qr-edit-modal');
                if (modal) modal.style.display = 'none';
            });
        }
        ...
        document.querySelectorAll('[data-action="delete-qr-code"]').forEach(function(btn) {
         ...
         if (downloadQrBtn) {
             downloadQrBtn.addEventListener('click', function() {
                 // Removed qr-image reference since we don't display QR in current tab
             });
         }
