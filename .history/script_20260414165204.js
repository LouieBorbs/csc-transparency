    ...
    renderAdminEvents: function() {
        ...
        if (events.length === 0) {
            ...
            for (var i = 0; i < filteredEvents.length; i++) {
                ...
                var mediaCount = e.media ? e.media.length : 0;
...
// Media carousel
                if (mediaCount > 0) {
                    var allMedia = e.media;
                    html += '<div class=\"media-carousel\" id=\"event-carousel-' + e.id + '\" style=\"position:relative;width:100%;max-height:400px;overflow:hidden;background:#1a1a1a;border-radius:8px;margin-bottom:12px;\">';
                    
                    var firstMedia = allMedia[0];
                    var isVideo = firstMedia && firstMedia.startsWith('data:video');
                    html += '<div class=\"media-slide\" style=\"width:100%;display:flex;align-items:center;justify-content:center;min-height:400px;\">';
                    if (isVideo) {
                        html += '<video controls style=\"max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;\"><source src=\"' + firstMedia + '\"></video></div>';
                    } else {
                        html += '<img src=\"' + firstMedia + '\" style=\"max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;cursor:pointer;object-fit:contain;\" onclick=\"App.viewAnnouncementImage(this)\"></div>';
                    }
                    
                    if (mediaCount > 1) {
                        html += '<button class=\"media-nav prev\" onclick=\"App.prevEventMedia(' + e.id + ')\" style=\"position:absolute;left:8px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);color:white;border:none;border-radius:50%;width:40px;height:40px;cursor:pointer;font-size:18px;z-index:10;display:flex;align-items:center;justify-content:center;\">&#10094;</button>' +
                            '<button class=\"media-nav next\" onclick=\"App.nextEventMedia(' + e.id + ')\" style=\"position:absolute;right:8px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);color:white;border:none;border-radius:50%;width:40px;height:40px;cursor:pointer;font-size:18px;z-index:10;display:flex;align-items:center;justify-content:center;\">&#10095;</button>';
                    }
                    html += '</div>';
                }
                ...
                html += '<div style="text-align:center;margin-bottom:12px;">' +
                    ...
                    '<p class="card-text mt-4">' + e.description + '</p>';
                
                // Comments section - expandable
                var comments = e.comments || [];
                html += '<div class="event-comments" style="margin-top:16px;border-top:1px solid var(--border-color);padding-top:12px;">' +
                    '<div class="comments-header" onclick="App.toggleEventComments(' + e.id + ')" style="cursor:pointer;display:flex;justify-content:space-between;align-items:center;font-weight:600;font-size:13px;margin-bottom:8px;">' +
                    '<span><i class="fas fa-comments"></i> Comments (' + comments.length + ')</span>' +
                    '<i class="fas fa-chevron-down" id="comment-icon-' + e.id + '"></i></div>' +
                    '<div class="comments-content" id="comments-content-' + e.id + '" style="display:none;max-height:200px;overflow-y:auto;">';
                
                if (comments.length > 0) {
                    comments.forEach(function(c) {
                        html += '<div class="comment-item" style="background:var(--bg-color);padding:8px;border-radius:6px;margin-bottom:6px;font-size:12px;">' +
                            '<div style="font-weight:600;font-size:11px;margin-bottom:2px;"><i class="fas fa-user"></i> ' + c.author + '</div>' +
                            '<div>' + c.content + '</div>' +
                            '<div style="font-size:10px;color:var(--text-light);">' + c.date + '</div>';
                        if (c.replies && c.replies.length > 0) {
                            c.replies.forEach(function(r) {
                                html += '<div style="margin-left:12px;margin-top:4px;padding:6px;background:white;border-radius:4px;border-left:2px solid var(--primary-color);font-size:11px;">' +
                                    '<div style="font-weight:600;color:var(--primary-color);font-size:10px;">Admin Reply:</div>' +
                ...
                // Add comment input
                html += '<div style="display:flex;gap:6px;margin-top:8px;"><input type="text" class="form-input" id="event-comment-' + e.id + '" placeholder="Write comment..." style="flex:1;font-size:12px;">' +
                    '<button class="btn btn-primary btn-sm" onclick="App.addEventComment(' + e.id + ')">Post</button></div>';
                
                if (self.currentUser.role === 'admin') {
                    html += '<div style="display:flex;gap:6px;margin-top:6px;"><input type="text" class="form-input" id="event-reply-' + e.id + '" placeholder="Reply as Admin..." style="flex:1;font-size:12px;">' +
                        '<button class="btn btn-success btn-sm" onclick="App.addEventReply(' + e.id + ')">Reply</button></div>';
                }
...
'<div class="form-group"><label class="form-label">Images/Videos (Multiple)</label>' +
            ...
            '<div class="file-upload-text">Click to upload <span>Images or Videos</span></div>' +
    ...
    renderAdminAnnouncements: function() {
        ...
        if (announcements.length === 0) {
            ...
            for (var i = 0; i < announcements.length; i++) {
                ...
                var mediaCount = (a.images ? a.images.length : 0) + (a.videos ? a.videos.length : 0);
                var currentIdx = 0;
                ...
                // Media carousel if multiple images/videos
                if (mediaCount > 0) {
                    var allMedia = (a.images || []).concat(a.videos || []);
                    html += '<div class="media-carousel" id="carousel-' + a.id + '" style="position:relative;width:100%;max-height:400px;overflow:hidden;background:#1a1a1a;">';
                    
                    // Display first media
                    var firstMedia = allMedia[0];
                    if (firstMedia) {
                        var isVideo = firstMedia.type === 'video' || firstMedia.startsWith('data:video');
                        html += '<div class="media-slide" style="width:100%;display:flex;align-items:center;justify-content:center;">';
                        if (isVideo) {
                            html += '<video controls style="max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;"><source src="' + firstMedia + '"></video></div>';
                        } else {
                            html += '<img src="' + firstMedia + '" class="announcement-media" data-announcement="' + a.id + '" data-index="0" style="max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;cursor:pointer;" onclick="App.viewAnnouncementImage(this)">';
                        }
                        html += '</div>';
                    }
                    ...
                    if (mediaCount > 1) {
                        html += '<button class="media-nav prev" onclick="App.prevMedia(' + a.id + ')" style="position:absolute;left:10px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);color:white;border:none;border-radius:50%;width:40px;height:40px;cursor:pointer;font-size:18px;z-index:10;">&#10094;</button>' +
                            '<button class="media-nav next" onclick="App.nextMedia(' + a.id + ')" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);color:white;border:none;border-radius:50%;width:40px;height:40px;cursor:pointer;font-size:18px;z-index:10;">&#10095;</button>' +
                            '<div class="media-dots" style="position:absolute;bottom:10px;left:50%;transform:translateX(-50%);display:flex;gap:8px;">';
                        for (var m = 0; m < mediaCount; m++) {
                            html += '<span class="media-dot" style="width:8px;height:8px;border-radius:50%;background:' + (m === 0 ? 'white' : 'rgba(255,255,255,0.5)') + ';"></span>';
                        }
                        html += '</div>';
                    }
                    html += '</div>';
                } else if (a.image) {
                    // Single image - show properly fitted
                    html += '<div style="width:100%;max-height:400px;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#1a1a1a;">' +
                        '<img src="' + a.image + '" class="announcement-media" data-announcement="' + a.id + '" data-index="0" style="max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;cursor:pointer;" onclick="App.viewAnnouncementImage(this)">' +
                        '</div>';
                } else if (a.video) {
                    html += '<div style="width:100%;max-height:400px;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#1a1a1a;">' +
                        '<video controls style="max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;"><source src="' + a.video + '"></video></div>';
                }
                ...
                html += '<div style="padding:16px;">' +
                    ...
                    '<p style="font-size:14px;line-height:1.6;margin-bottom:12px;white-space:pre-wrap;word-wrap:break-word;">' + a.content + '</p>';
                ...
                if (a.attachments && a.attachments.length > 0) {
                    ...
                    a.attachments.forEach(function(att, idx) {
                        var fileName = att.name || 'Attachment ' + (idx + 1);
                        var fileData = att.data || att;
                        html += '<a href="' + fileData + '" download="' + fileName + '" target="_blank" class="file-item" style="padding:8px 12px;background:var(--bg-color);border-radius:6px;display:flex;align-items:center;gap:8px;text-decoration:none;color:inherit;cursor:pointer;">' +
                            '<i class="fas fa-paperclip"></i><span>' + fileName + '</span></a>';
                    });
                    html += '</div></div>';
                }
                ...
                html += '<div class="comments-section" id="comments-' + a.id + '" style="margin-top:16px;padding-top:12px;border-top:1px solid var(--border-color);">' +
                    '<div style="font-weight:600;margin-bottom:8px;">Comments (' + comments.length + ')</div>';
                
                // Show existing comments
                if (comments.length > 0) {
                    html += '<div class="comments-list" style="max-height:200px;overflow-y:auto;margin-bottom:12px;">';
                    comments.forEach(function(c) {
                        html += '<div class="comment-item" style="background:var(--bg-color);padding:10px;border-radius:8px;margin-bottom:8px;">' +
                            '<div style="font-weight:600;font-size:12px;margin-bottom:4px;"><i class="fas fa-user"></i> ' + c.author + '</div>' +
                            '<div style="font-size:13px;margin-bottom:4px;">' + c.content + '</div>' +
                            '<div style="font-size:11px;color:var(--text-light);">' + c.date + '</div>';
                        // Show replies
                        if (c.replies && c.replies.length > 0) {
                            c.replies.forEach(function(r) {
                                html += '<div class="reply-item" style="margin-left:20px;margin-top:8px;padding:8px;background:white;border-radius:6px;border-left:3px solid var(--primary-color);">' +
                                    '<div style="font-weight:600;font-size:11px;color:var(--primary-color);margin-bottom:2px;">Admin Reply:</div>' +
                                    '<div style="font-size:12px;">' + r.content + '</div>' +
                ...
                // Add comment form
                html += '<div class="add-comment" style="display:flex;gap:8px;margin-bottom:8px;">' +
                    '<input type="text" class="form-input" id="comment-input-' + a.id + '" placeholder="Write a comment..." style="flex:1;font-size:13px;">' +
                    '<button class="btn btn-primary btn-sm" onclick="App.addComment(' + a.id + ')">Post</button></div>';
                
                // Admin reply input (only show for admin)
                if (self.currentUser.role === 'admin') {
                    html += '<div class="admin-reply" style="margin-top:8px;padding:8px;background:var(--bg-color);border-radius:8px;">' +
                        '<div style="font-size:12px;font-weight:600;margin-bottom:4px;">Reply as Admin:</div>' +
                        '<div style="display:flex;gap:8px;">' +
                        '<input type="text" class="form-input" id="reply-input-' + a.id + '" placeholder="Write reply..." style="flex:1;font-size:12px;">' +
                        '<button class="btn btn-success btn-sm" onclick="App.addReply(' + a.id + ')">Reply</button></div></div>';
                }
                ...
                html += '<div style="display:flex;gap:8px;align-items:center;border-top:1px solid var(--border-color);padding-top:12px;margin-top:12px;">' +
                    '<button class="btn btn-' + (likesCount > 0 ? 'success' : 'secondary') + ' btn-sm" onclick="App.toggleLike(' + a.id + ')"><i class="fas fa-thumbs-up"></i> ' + likesCount + ' Likes</button>' +
                    '<button class="btn btn-secondary btn-sm" data-action="edit-announcement" data-id="' + a.id + '"><i class="fas fa-edit"></i> Edit</button>' +
    ...
    renderCreateAnnouncementModal: function() {
        return '<div id="announcement-modal" class="modal" style="display:none;align-items:center;justify-content:center;">' +
            ...
            '<input type="file" id="announcement-image-input" accept="image/*" multiple style="display:none;"></div>' +
    ...
    showScanQrModal: function() {
        var self = this;
        var html = '<div id="scan-qr-modal" class="modal" style="display:flex;align-items:center;justify-content:center;">' +
            '<div class="modal-content" style="max-width:400px;text-align:center;">' +
            '<div class="modal-header"><h3>Scan QR Code</h3><button class="modal-close" id="close-scan-qr">&times;</button></div>' +
            '<div style="padding:24px;">' +
            '<p class="text-sm text-muted" style="margin-bottom:16px;">Point your camera at an event QR code to record attendance</p>' +
            '<div style="width:100%;height:250px;background:var(--bg-color);border-radius:var(--radius-md);display:flex;align-items:center;justify-content:center;margin-bottom:16px;">' +
            '<div style="text-align:center;"><i class="fas fa-qrcode" style="font-size:64px;color:var(--text-light);"></i>' +
            '<p class="text-sm text-muted" style="margin-top:12px;">Camera access required</p></div></div>' +
            '<input type="text" class="form-input" id="qr-code-input" placeholder="Or enter QR code manually" style="margin-bottom:12px;">' +
            '<button class="btn btn-primary btn-block" id="submit-qr-code">Submit Code</button>' +
            '</div></div></div>';
        ...
        var closeBtn = document.getElementById('close-scan-qr');
        var modal = document.getElementById('scan-qr-modal');
        if (closeBtn && modal) {
            closeBtn.addEventListener('click', function() {
                modal.remove();
            });
        ...
        var submitBtn = document.getElementById('submit-qr-code');
        if (submitBtn) {
            submitBtn.addEventListener('click', function() {
                var code = document.getElementById('qr-code-input').value;
                if (code) {
    ...
    processQrCode: function(code) {
        ...
        for (var i = 0; i < events.length; i++) {
            var e = events[i];
            if (e.qrCode === code || ('CSC:' + e.qrCode) === code || code.includes(e.id + '')) {
                if (!e.attendees) e.attendees = [];
                var alreadyMarked = false;
                for (var j = 0; j < e.attendees.length; j++) {
                    if (e.attendees[j].email === self.currentUser.email) {
                        alreadyMarked = true;
                        break;
                    }
                }
                if (!alreadyMarked) {
                    ...
                    found = true;
                    self.saveData();
                    alert('Attendance marked for: ' + e.title);
                } else {
        ...
        if (!found) {
            alert('Invalid QR code. Please try again or contact event organizer.');
        }
    },

    // Remove Event Media
    removeEventMedia: function(index) {
        if (this.registerData && this.registerData.eventMedia && this.registerData.eventMedia[index] !== undefined) {
            this.registerData.eventMedia.splice(index, 1);
            var eventMediaPreview = document.getElementById('event-media-preview');
            if (eventMediaPreview) {
                var previews = '<div style="display:flex;flex-wrap:wrap;gap:4px;">';
                var maxShow = Math.min(this.registerData.eventMedia.length, 6);
                for (var i = 0; i < maxShow; i++) {
                    var img = this.registerData.eventMedia[i];
                    var isVideo = img.startsWith('data:video');
                    previews += '<div style="position:relative;width:50px;height:50px;overflow:hidden;border-radius:4px;display:inline-block;">' +
                        (isVideo ? '<video src="' + img + '" style="width:100%;height:100%;object-fit:cover;">' :
                        '<img src="' + img + '" style="width:100%;height:100%;object-fit:cover;">') +
                        '<button type="button" onclick="event.stopPropagation();App.removeEventMedia(' + i + ')" style="position:absolute;top:0;right:0;background:red;color:white;border:none;border-radius:0 4px 0 4px;width:16px;height:16px;font-size:10px;cursor:pointer;line-height:1;">&times;</button></div>';
                }
                if (this.registerData.eventMedia.length > 6) {
                    previews += '<span style="font-size:11px;color:var(--text-light);align-self:center;margin-left:4px;">+' + (this.registerData.eventMedia.length - 6) + ' more</span>';
                }
                previews += '</div>';
                if (this.registerData.eventMedia.length === 0) {
                    eventMediaPreview.innerHTML = '';
                    eventMediaPreview.classList.remove('show');
                    document.getElementById('event-media-upload').classList.remove('has-file');
                } else {
                    eventMediaPreview.innerHTML = previews;
                }
            }
        }
    },

    // Event Media Carousel Navigation
    currentEventMediaIndex: {},
    nextEventMedia: function(eventId) {
        var event = this.data.events.find(function(e) { return e.id == eventId; });
        if (!event || !event.media || event.media.length === 0) return;
        
        var total = event.media.length;
        var currentIdx = this.currentEventMediaIndex[eventId] || 0;
        currentIdx = (currentIdx + 1) % total;
        this.currentEventMediaIndex[eventId] = currentIdx;
        
        this.updateEventMediaDisplay(eventId, currentIdx, event.media);
    },

    prevEventMedia: function(eventId) {
        var event = this.data.events.find(function(e) { return e.id == eventId; });
        if (!event || !event.media || event.media.length === 0) return;
    ...
    updateEventMediaDisplay: function(eventId, index, allMedia) {
        var carousel = document.getElementById('event-carousel-' + eventId);
        if (!carousel) return;
        
        var media = allMedia[index];
        var isVideo = media && media.startsWith('data:video');
        
        var content = '<div class="media-slide" style="width:100%;display:flex;align-items:center;justify-content:center;">';
        if (isVideo) {
            content += '<video controls style="max-width:100%;max-height:250px;width:auto;height:auto;display:block;margin:0 auto;"><source src="' + media + '"></video></div>';
        } else {
            content += '<img src="' + media + '" style="max-width:100%;max-height:250px;width:auto;height:auto;display:block;margin:0 auto;cursor:pointer;" onclick="App.viewAnnouncementImage(this)"></div>';
        }
        content += '</div>';
        
        var navButtons = carousel.querySelectorAll('.media-nav');
        var existingSlide = carousel.querySelector('.media-slide');
        if (existingSlide) existingSlide.outerHTML = content;
        
        var navDots = carousel.querySelector('.media-dots');
        if (navDots) {
            navDots.querySelectorAll('.media-dot').forEach(function(d, i) {
    ...
    // Event Comments
    toggleEventComments: function(eventId) {
        var content = document.getElementById('comments-content-' + eventId);
        var icon = document.getElementById('comment-icon-' + eventId);
        if (content) {
    ...
    addEventComment: function(eventId) {
        var input = document.getElementById('event-comment-' + eventId);
    ...
    addEventReply: function(eventId) {
        var input = document.getElementById('event-reply-' + eventId);
        var content = input ? input.value.trim() : '';
        ...
        if (!event || !event.comments || event.comments.length === 0) {
            alert('No comments to reply to');
            return;
        }
        
        var lastComment = event.comments[event.comments.length - 1];
        if (!lastComment.replies) lastComment.replies = [];
        
        lastComment.replies.push({
            content: content,
            date: new Date().toISOString().split('T')[0] + ' ' + new Date().toTimeString().slice(0, 5),
            fromAdmin: true
        });
    ...
    // Announcement Image Viewer (Lightbox)
    viewAnnouncementImage: function(imgEl) {
    ...
    currentMediaIndex: {},
    nextMedia: function(announcementId) {
        var announcement = this.data.announcements.find(function(a) { return a.id == announcementId; });
        if (!announcement) return;
        
        var allMedia = (announcement.images || []).concat(announcement.videos || []);
        var total = allMedia.length;
        if (total === 0) return;
        
        var currentIdx = this.currentMediaIndex[announcementId] || 0;
        currentIdx = (currentIdx + 1) % total;
        this.currentMediaIndex[announcementId] = currentIdx;
        
        this.updateMediaDisplay(announcementId, currentIdx, allMedia);
    },

    prevMedia: function(announcementId) {
        var announcement = this.data.announcements.find(function(a) { return a.id == announcementId; });
        if (!announcement) return;
    ...
    updateMediaDisplay: function(announcementId, index, allMedia) {
        var carousel = document.getElementById('carousel-' + announcementId);
        if (!carousel) return;
        
        var media = allMedia[index];
        var isVideo = media && (media.type === 'video' || (typeof media === 'string' && media.startsWith('data:video')));
        
        var content = '<div class="media-slide" style="width:100%;display:flex;align-items:center;justify-content:center;">';
        if (isVideo) {
            content += '<video controls style="max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;"><source src="' + media + '"></video></div>';
        } else {
            content += '<img src="' + media + '" class="announcement-media" data-announcement="' + announcementId + '" data-index="' + index + '" style="max-width:100%;max-height:400px;width:auto;height:auto;display:block;margin:0 auto;cursor:pointer;" onclick="App.viewAnnouncementImage(this)"></div>';
        }
        content += '</div>';
        
        // Update navigation buttons
        var prevBtn = carousel.querySelector('.media-nav.prev');
        var nextBtn = carousel.querySelector('.media-nav.next');
        var dots = carousel.querySelectorAll('.media-dot');
        
        // Re-preend media content after nav buttons
        var navButtons = carousel.querySelectorAll('.media-nav');
        var existingSlides = carousel.querySelectorAll('.media-slide');
        existingSlides.forEach(function(s) { s.remove(); });
        
        if (navButtons.length > 0) {
            carousel.insertBefore(document.createElement('div'), navButtons[0]).outerHTML = content;
        } else {
            carousel.innerHTML = content + carousel.innerHTML;
        }
    ...
    // Comments
    addComment: function(announcementId) {
        var input = document.getElementById('comment-input-' + announcementId);
        var content = input ? input.value.trim() : '';
    ...
    addReply: function(announcementId) {
        var input = document.getElementById('reply-input-' + announcementId);
        var content = input ? input.value.trim() : '';
        ...
        var announcement = this.data.announcements.find(function(a) { return a.id == announcementId; });
        if (!announcement || !announcement.comments || announcement.comments.length === 0) {
            alert('No comments to reply to');
            return;
        }
        
        var lastComment = announcement.comments[announcement.comments.length - 1];
        if (!lastComment.replies) lastComment.replies = [];
    ...
    toggleLike: function(announcementId) {
        var announcement = this.data.announcements.find(function(a) { return a.id == announcementId; });
        if (!announcement) return;
        
        if (!announcement.likes) announcement.likes = [];
        
        var userEmail = this.currentUser.email;
        var likeIndex = announcement.likes.indexOf(userEmail);
        
        if (likeIndex > -1) {
            announcement.likes.splice(likeIndex, 1);
        } else {
            announcement.likes.push(userEmail);
        }
    ...
    renderStudentAnnouncements: function() {
        ...
        this.data.announcements.forEach(function(a) {
            ...
            html += '<div class="announcement-item" style="background:var(--bg-white);border-radius:var(--radius-md);border:1px solid var(--border-color);margin-bottom:16px;overflow:hidden;padding:16px;">' +
                '<div class="announcement-date" style="font-size:12px;color:var(--text-light);margin-bottom:8px;">' + a.date + (a.pinned ? ' <span class="badge badge-warning">Pinned</span>' : '') + '</div>' +
                '<h3 class="announcement-title" style="font-size:18px;font-weight:600;margin-bottom:12px;">' + a.title + '</h3>';
            
            // Media carousel
            if (mediaCount > 0) {
                var allMedia = (a.images || []).concat(a.videos || []);
                html += '<div class="media-carousel" id="carousel-student-' + a.id + '" style="position:relative;width:100%;max-height:350px;overflow:hidden;background:#1a1a1a;border-radius:8px;margin-bottom:12px;">';
                
                var firstMedia = allMedia[0];
                var isVideo = firstMedia && (firstMedia.type === 'video' || (typeof firstMedia === 'string' && firstMedia.startsWith('data:video')));
                html += '<div class="media-slide" style="width:100%;display:flex;align-items:center;justify-content:center;">';
                if (isVideo) {
                    html += '<video controls style="max-width:100%;max-height:350px;width:auto;height:auto;display:block;margin:0 auto;"><source src="' + firstMedia + '"></video></div>';
                } else {
                    html += '<img src="' + firstMedia + '" style="max-width:100%;max-height:350px;width:auto;height:auto;display:block;margin:0 auto;cursor:pointer;" onclick="App.viewAnnouncementImage(this)"></div>';
                }
                html += '</div>';
                
                if (mediaCount > 1) {
                    html += '<button class="media-nav prev" onclick="App.prevMedia(' + a.id + ')" style="position:absolute;left:10px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);color:white;border:none;border-radius:50%;width:36px;height:36px;cursor:pointer;font-size:16px;z-index:10;">&#10094;</button>' +
                        '<button class="media-nav next" onclick="App.nextMedia(' + a.id + ')" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);color:white;border:none;border-radius:50%;width:36px;height:36px;cursor:pointer;font-size:16px;z-index:10;">&#10095;</button>';
                }
                html += '</div>';
            } else if (a.image) {
                html += '<div style="width:100%;max-height:350px;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#1a1a1a;border-radius:8px;margin-bottom:12px;">' +
                    '<img src="' + a.image + '" style="max-width:100%;max-height:350px;width:auto;height:auto;display:block;margin:0 auto;cursor:pointer;" onclick="App.viewAnnouncementImage(this)"></div>';
            } else if (a.video) {
                html += '<div style="width:100%;max-height:350px;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#1a1a1a;border-radius:8px;margin-bottom:12px;">' +
                    '<video controls style="max-width:100%;max-height:350px;width:auto;height:auto;display:block;margin:0 auto;"><source src="' + a.video + '"></video></div>';
            }
            
            html += '<p class="announcement-content" style="font-size:14px;line-height:1.6;margin-bottom:12px;white-space:pre-wrap;word-wrap:break-word;">' + a.content + '</p>';
            ...
            if (a.attachments && a.attachments.length > 0) {
                ...
                a.attachments.forEach(function(att, idx) {
                    var fileName = att.name || 'Attachment ' + (idx + 1);
                    var fileData = att.data || att;
                    html += '<a href="' + fileData + '" download="' + fileName + '" target="_blank" style="padding:8px 12px;background:var(--bg-color);border-radius:6px;display:flex;align-items:center;gap:8px;text-decoration:none;color:inherit;">' +
                        '<i class="fas fa-paperclip"></i><span>' + fileName + '</span></a>';
                });
                html += '</div></div>';
            }
            ...
            // Comments section
            var comments = a.comments || [];
            html += '<div class="comments-section" style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border-color);">' +
                '<div style="font-weight:600;margin-bottom:8px;font-size:13px;">Comments (' + comments.length + ')</div>';
            
            if (comments.length > 0) {
                html += '<div class="comments-list" style="max-height:150px;overflow-y:auto;margin-bottom:8px;">';
                comments.forEach(function(c) {
                    html += '<div class="comment-item" style="background:var(--bg-color);padding:8px;border-radius:6px;margin-bottom:6px;font-size:13px;">' +
                        '<div style="font-weight:600;font-size:11px;margin-bottom:2px;"><i class="fas fa-user"></i> ' + c.author + '</div>' +
                        '<div>' + c.content + '</div>';
                    if (c.replies && c.replies.length > 0) {
                        c.replies.forEach(function(r) {
                            html += '<div style="margin-left:12px;margin-top:4px;padding:6px;background:white;border-radius:4px;border-left:2px solid var(--primary-color);font-size:12px;">' +
                                '<div style="font-weight:600;color:var(--primary-color);font-size:10px;">Admin Reply:</div>' +
                                '<div>' + r.content + '</div></div>';
                        });
                    }
            ...
            // Add comment input
            html += '<div style="display:flex;gap:8px;"><input type="text" class="form-input" id="student-comment-' + a.id + '" placeholder="Write a comment..." style="flex:1;font-size:13px;">' +
                '<button class="btn btn-primary btn-sm" onclick="App.addComment(' + a.id + ')">Post</button></div>';
            
            html += '</div>' +
                '<div class="announcement-actions" style="display:flex;gap:8px;margin-top:12px;padding-top:12px;border-top:1px solid var(--border-color);">' +
                '<button class="btn btn-' + (hasLiked ? 'success' : 'secondary') + ' btn-sm" onclick="App.toggleLike(' + a.id + ')"><i class="fas fa-thumbs-up"></i> ' + likesCount + '</button>' +
    ...
    attachDashboardListeners: function(role) {
        ...
        // Calendar Navigation
        var calendarNavBtns = document.querySelectorAll('.calendar-nav-btn');
        Array.prototype.forEach.call(calendarNavBtns, function(btn) {
            btn.addEventListener('click', function() {
                var action = btn.dataset.calendar;
                if (action === 'prev') {
        ...
        var chatbotMsgs = document.getElementById('chatbot-messages');
        if (chatbotSend && chatbotInput && chatbotMsgs) {
            chatbotSend.addEventListener('click', function() {
                var msg = chatbotInput.value.trim();
                if (msg && chatbotMsgs) {
                    chatbotMsgs.innerHTML += '<div style="background:white;padding:12px;border-radius:8px;margin-bottom:8px;max-width:80%;margin-left:auto;text-align:right;"><strong>You:</strong> ' + msg + '</div>';
                    var responses = [
                        { keywords: ['hello', 'hi', 'hey'], response: 'Hello! How can I help you today?' },
                        { keywords: ['help'], response: 'I can help you with navigating the dashboard, checking events, and more. What do you need?' },
                        { keywords: ['event', 'events', 'calendar'], response: 'You can view upcoming events in the Events tab. Check the calendar on the right sidebar.' },
                        { keywords: ['file', 'files', 'download'], response: 'Files are available in the Files tab. You can download reports and documents there.' },
                        { keywords: ['finance', 'funds', 'budget'], response: 'Financial information is available in the Finance tab for approved users.' },
                        { keywords: ['poll', 'vote'], response: 'You can participate in polls in the Pols tab when they are active.' },
                        { keywords: ['complaint', 'report issue'], response: 'You can submit complaints in the Complaints tab. Describe your issue and we will address it.' },
                        { keywords: ['thank', 'thanks'], response: 'You are welcome! Is there anything else I can help with?' }
                    ];
                    var matched = responses.find(function(r) { return r.keywords.some(function(k) { return msg.toLowerCase().includes(k); }); });
                    var reply = matched ? matched.response : 'I understand your message. For more specific help, please contact the admin through the Complaints tab.';
                    chatbotMsgs.innerHTML += '<div style="background:white;padding:12px;border-radius:8px;margin-bottom:8px;max-width:80%;"><strong>Assistant:</strong> ' + reply + '</div>';
        ...
        Array.prototype.forEach.call(editHeadlineBtns, function(btn) {
            ...
            btn.addEventListener('click', function() {
                var h = self.data.headlines.find(function(head) { return head.id == btn.dataset.id; });
                if (h && headlineModal) {
                    headlineForm.dataset.editId = h.id;
                    document.getElementById('headline-title').value = h.title;
                    document.getElementById('headline-content').value = h.content;
                    headlineModal.style.display = 'flex';
                }
            });
        });
        ...
        if (createEventBtn && eventModal) {
            createEventBtn.addEventListener('click', function() {
                ...
                document.getElementById('event-category-select').value = '';
                document.getElementById('event-category-custom').value = '';
                document.getElementById('event-status').value = 'upcoming';
                document.getElementById('event-batch').value = '';
                document.getElementById('event-date').value = '';
                document.getElementById('event-time').value = '';
                document.getElementById('event-location').value = '';
                document.getElementById('event-description').value = '';
                document.getElementById('event-qr-code').value = '';
                document.getElementById('event-evaluation-link').value = '';
                document.getElementById('event-pin').checked = false;
        ...
        if (eventForm && !eventForm.dataset.listenerAttached) {
            ...
            eventForm.addEventListener('submit', function(e) {
                ...
                var category = document.getElementById('event-category-select').value || document.getElementById('event-category-custom').value;
                ...
                var qrCode = document.getElementById('event-qr-code').value;
                var evaluationLink = document.getElementById('event-evaluation-link').value;
                var pinned = document.getElementById('event-pin').checked;
                var editId = eventForm.dataset.editId;
                
                // Get media files from registerData
                var mediaFiles = self.registerData ? self.registerData.eventMedia || [] : [];
                
                if (editId) {
                    var idx = self.data.events.findIndex(function(ev) { return ev.id == editId; });
                    var existing = self.data.events[idx];
                    if (idx > -1) {
                        var existingMedia = existing.media || [];
                        self.data.events[idx] = Object.assign({}, existing, {
                            title: title, category: category, status: status, batch: batch,
                            date: date, time: time, location: location, description: description,
                            qrCode: qrCode, evaluationLink: evaluationLink, pinned: pinned,
                            media: mediaFiles.length > 0 ? mediaFiles : existingMedia
                        });
                    }
                } else {
                    self.data.events.push({
                        ...
                        qrCode: qrCode, evaluationLink: evaluationLink, pinned: pinned,
                        media: mediaFiles,
                        rsvps: [], attendees: [], evaluations: [], comments: []
                    });
                }
                
                // Clear temporary data
                self.registerData = self.registerData || {};
                self.registerData.eventMedia = [];
        ...
        // Event Media Upload (Multiple)
        var eventMediaInput = document.getElementById('event-media-input');
        var eventMediaUpload = document.getElementById('event-media-upload');
        var eventMediaPreview = document.getElementById('event-media-preview');
        if (eventMediaInput && eventMediaUpload) {
            eventMediaUpload.addEventListener('click', function() { eventMediaInput.click(); });
            eventMediaInput.addEventListener('change', function(e) {
                self.registerData = self.registerData || {};
                if (!self.registerData.eventMedia) self.registerData.eventMedia = [];
                ...
                for (var i = 0; i < files.length; i++) {
                    (function(file) {
                        var reader = new FileReader();
                        reader.onload = function(evt) {
                            self.registerData.eventMedia.push(evt.target.result);
                            loaded++;
                            
                            if (loaded === files.length) {
                                if (eventMediaPreview) {
                                    var previews = '<div style="display:flex;flex-wrap:wrap;gap:4px;">';
                                    self.registerData.eventMedia.slice(0, 6).forEach(function(img, idx) {
                                        var isVideo = img.startsWith('data:video');
                                        previews += '<div style="position:relative;width:50px;height:50px;overflow:hidden;border-radius:4px;display:inline-block;">' +
                                            (isVideo ? '<video src="' + img + '" style="width:100%;height:100%;object-fit:cover;">' :
                                            '<img src="' + img + '" style="width:100%;height:100%;object-fit:cover;">') +
                                            '<button type="button" onclick="event.stopPropagation();App.removeEventMedia(' + idx + ')" style="position:absolute;top:0;right:0;background:red;color:white;border:none;border-radius:0 4px 0 4px;width:16px;height:16px;font-size:10px;cursor:pointer;line-height:1;">&times;</button></div>';
                                    });
                                    if (self.registerData.eventMedia.length > 6) {
                                        previews += '<span style="font-size:11px;color:var(--text-light);align-self:center;">+' + (self.registerData.eventMedia.length - 6) + ' more</span>';
                                    }
                                    previews += '</div>';
                                    eventMediaPreview.innerHTML = previews;
                                    eventMediaPreview.classList.add('show');
                                }
                                eventMediaUpload.classList.add('has-file');
                            }
                        };
                        reader.readAsDataURL(file);
                    })(files[i]);
        ...
        // Edit Event
        var editEventBtns = document.querySelectorAll('[data-action="edit-event"]');
        Array.prototype.forEach.call(editEventBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                var ev = self.data.events.find(function(e) { return e.id == btn.dataset.id; });
                if (ev && eventModal) {
                    eventForm.dataset.editId = ev.id;
                    document.getElementById('event-title').value = ev.title || '';
                    document.getElementById('event-category-select').value = ev.category || '';
                    document.getElementById('event-status').value = ev.status || 'upcoming';
                    document.getElementById('event-batch').value = ev.batch || '';
                    document.getElementById('event-date').value = ev.date || '';
                    document.getElementById('event-time').value = ev.time || '';
                    document.getElementById('event-location').value = ev.location || '';
                    document.getElementById('event-description').value = ev.description || '';
                    document.getElementById('event-qr-code').value = ev.qrCode || '';
                    document.getElementById('event-evaluation-link').value = ev.evaluationLink || '';
                    document.getElementById('event-pin').checked = ev.pinned || false;
                    
                    // Load existing media into preview
                    var eventMediaPreview = document.getElementById('event-media-preview');
                    if (eventMediaPreview && ev.media && ev.media.length > 0) {
                        self.registerData = self.registerData || {};
                        self.registerData.eventMedia = ev.media.slice();
                        var previews = '<div style="display:flex;flex-wrap:wrap;gap:4px;">';
                        ev.media.slice(0, 6).forEach(function(img, idx) {
                            var isVideo = img.startsWith('data:video');
                            previews += '<div style="position:relative;width:50px;height:50px;overflow:hidden;border-radius:4px;">' +
                                (isVideo ? '<video src="' + img + '" style="width:100%;height:100%;object-fit:cover;">' :
                                '<img src="' + img + '" style="width:100%;height:100%;object-fit:cover;">') +
                                '<button type="button" onclick="App.removeEventMedia(' + idx + ')" style="position:absolute;top:0;right:0;background:red;color:white;border:none;border-radius:0 4px 0 4px;width:16px;height:16px;font-size:10px;cursor:pointer;">&times;</button></div>';
                        });
                        if (ev.media.length > 6) {
                            previews += '<span style="font-size:11px;color:var(--text-light);align-self:center;">+' + (ev.media.length - 6) + ' more</span>';
                        }
                        previews += '</div>';
                        eventMediaPreview.innerHTML = previews;
                        eventMediaPreview.classList.add('show');
                        document.getElementById('event-media-upload').classList.add('has-file');
                    }
        ...
        // Announcements (Admin) - Create Announcement
        var createAnnouncementBtn = document.getElementById('create-announcement-btn');
        var announcementModal = document.getElementById('announcement-modal');
        var closeAnnouncementModal = document.getElementById('close-announcement-modal');
        ...
        if (createAnnouncementBtn && announcementModal) {
            createAnnouncementBtn.addEventListener('click', function() {
                ...
                document.getElementById('announcement-pin').checked = false;
        ...
        // Announcement Image Upload (Multiple)
        var announcementImageInput = document.getElementById('announcement-image-input');
        var announcementImageUpload = document.getElementById('announcement-image-upload');
        var announcementImagePreview = document.getElementById('announcement-image-preview');
        if (announcementImageInput && announcementImageUpload) {
            announcementImageUpload.addEventListener('click', function() { announcementImageInput.click(); });
            announcementImageInput.addEventListener('change', function(e) {
                self.registerData = self.registerData || {};
                if (!self.registerData.announcementImages) self.registerData.announcementImages = [];
                ...
                for (var i = 0; i < files.length; i++) {
                    (function(file) {
                        var reader = new FileReader();
                        reader.onload = function(evt) {
                            self.registerData.announcementImages.push(evt.target.result);
                            loaded++;
                            ...
                            if (loaded === files.length) {
                                ...
                                if (announcementImagePreview) {
                                    var previews = '';
                                    self.registerData.announcementImages.slice(0, 4).forEach(function(img, idx) {
                                        previews += '<img src="' + img + '" style="width:60px;height:60px;object-fit:cover;border-radius:4px;margin-right:4px;">';
                                    });
                                    if (self.registerData.announcementImages.length > 4) {
                                        previews += '<span style="font-size:12px;color:var(--text-light);">+' + (self.registerData.announcementImages.length - 4) + ' more</span>';
                                    }
                                    announcementImagePreview.innerHTML = previews;
                                    announcementImagePreview.classList.add('show');
                                }
                                announcementImageUpload.classList.add('has-file');
                            }
                        };
                        reader.readAsDataURL(file);
                    })(files[i]);
        ...
        // Announcement Video Upload (Multiple)
        var announcementVideoInput = document.getElementById('announcement-video-input');
        var announcementVideoUpload = document.getElementById('announcement-video-upload');
        var announcementVideoPreview = document.getElementById('announcement-video-preview');
        if (announcementVideoInput && announcementVideoUpload) {
            announcementVideoUpload.addEventListener('click', function() { announcementVideoInput.click(); });
            announcementVideoInput.addEventListener('change', function(e) {
                self.registerData = self.registerData || {};
                if (!self.registerData.announcementVideos) self.registerData.announcementVideos = [];
                ...
                for (var i = 0; i < files.length; i++) {
                    (function(file) {
                        var reader = new FileReader();
                        reader.onload = function(evt) {
                            self.registerData.announcementVideos.push(evt.target.result);
                            loaded++;
                            
                            if (loaded === files.length) {
                                if (announcementVideoPreview) {
                                    var previews = '<div style="font-size:12px;color:var(--success-color);">' + self.registerData.announcementVideos.length + ' video(s) selected</div>';
                                    announcementVideoPreview.innerHTML = previews;
                                    announcementVideoPreview.classList.add('show');
                                }
                                announcementVideoUpload.classList.add('has-file');
                            }
                        };
                        reader.readAsDataURL(file);
                    })(files[i]);
        ...
        var announcementFilesUpload = document.getElementById('announcement-files-upload');
        var announcementFilesPreview = document.getElementById('announcement-files-preview');
        if (announcementFilesInput && announcementFilesUpload) {
            announcementFilesUpload.addEventListener('click', function() { announcementFilesInput.click(); });
            announcementFilesInput.addEventListener('change', function(e) {
                var files = this.files;
                if (files.length > 0) {
                    self.registerData = self.registerData || {};
                    var attachments = [];
                    ...
                    for (var i = 0; i < files.length; i++) {
                        (function(file) {
                            ...
                            reader.onload = function(evt) {
                                ...
                                loaded++;
                                if (loaded === files.length) {
                                    self.registerData.announcementAttachments = attachments;
                                    if (announcementFilesPreview) {
                                        announcementFilesPreview.innerHTML = '<div style="font-size:12px;color:var(--success-color);">' + files.length + ' file(s) attached</div>';
                                        announcementFilesPreview.classList.add('show');
                                    }
                                    announcementFilesUpload.classList.add('has-file');
                                }
                            };
                            reader.readAsDataURL(file);
                        })(files[i]);
        ...
        var editAnnouncementBtns = document.querySelectorAll('[data-action="edit-announcement"]');
        Array.prototype.forEach.call(editAnnouncementBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                var a = self.data.announcements.find(function(ann) { return ann.id == btn.dataset.id; });
                if (a && announcementModal) {
                    document.getElementById('announcement-title').value = a.title || '';
                    document.getElementById('announcement-content').value = a.content || '';
                    document.getElementById('announcement-pin').checked = a.pinned || false;
                    announcementForm.dataset.editId = a.id;
                    announcementModal.style.display = 'flex';
                }
            });
        });
        ...
        if (announcementForm) {
            // Update original submit handler to support edit
            var originalSubmit = announcementForm.dataset.listenerAttached;
            announcementForm.addEventListener('submit', function(e) {
                ...
                var pinned = document.getElementById('announcement-pin').checked;
                var editId = announcementForm.dataset.editId;
                
                // Get media files from registerData
                var images = self.registerData ? self.registerData.announcementImages : [];
                var videos = self.registerData ? self.registerData.announcementVideos : [];
                var attachments = self.registerData ? self.registerData.announcementAttachments : [];
                
                if (editId) {
                    var idx = self.data.announcements.findIndex(function(a) { return a.id == editId; });
                    var existing = self.data.announcements[idx];
                    if (idx > -1) {
                        // Convert old single image/video to new array format if needed
                        var existingImages = existing.images || (existing.image ? [existing.image] : []);
                        var existingVideos = existing.videos || (existing.video ? [existing.video] : []);
                        
                        self.data.announcements[idx] = Object.assign({}, existing, {
                            title: title, content: content, pinned: pinned,
                            images: images.length > 0 ? images : existingImages,
                            videos: videos.length > 0 ? videos : existingVideos,
                            attachments: attachments.length > 0 ? attachments : existing.attachments
                        });
                    }
                } else {
                    // Convert old single image/video to array format for backward compatibility
                    var oldImage = self.registerData ? self.registerData.announcementImage : null;
                    var oldVideo = self.registerData ? self.registerData.announcementVideo : null;
                    
                    self.data.announcements.push({
                        id: Date.now(),
                        title: title,
                        content: content,
                        date: new Date().toISOString().split('T')[0],
                        ...
                        readBy: [],
                        comments: [],
                        image: oldImage, // For backward compatibility
                        video: oldVideo, // For backward compatibility
                        images: images.length > 0 ? images : (oldImage ? [oldImage] : []),
                        videos: videos.length > 0 ? videos : (oldVideo ? [oldVideo] : []),
                ...
                self.registerData.announcementImages = [];
                self.registerData.announcementVideos = [];
                self.registerData.announcementAttachments = [];
                
                self.saveData();
                announcementModal.style.display = 'none';
                self.renderAdminDashboard();
            });
            announcementForm.dataset.listenerAttached = 'true';
        }

        // Pin Announcement
        var pinAnnouncementBtns = document.querySelectorAll('[data-action="pin-announcement"]');
        Array.prototype.forEach.call(pinAnnouncementBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                var a = self.data.announcements.find(function(ann) { return ann.id == btn.dataset.id; });
        ...
        var togglePollBtns = document.querySelectorAll('[data-action="toggle-poll"]');
        Array.prototype.forEach.call(togglePollBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
        ...
        // Delete Poll
        var deletePollBtns = document.querySelectorAll('[data-action="delete-poll"]');
        Array.prototype.forEach.call(deletePollBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                if (confirm('Delete this poll?')) {
                    self.data.polls = self.data.polls.filter(function(p) { return p.id != btn.dataset.id; });
                    self.saveData();
                    self.renderAdminDashboard();
                }
            });
        ...
        Array.prototype.forEach.call(pinAnnouncementBtns, function(btn) {
            ...
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                var announcement = self.data.announcements.find(function(a) { return a.id == btn.dataset.id; });
                if (announcement) {
        ...
        // Delete Announcement
        var deleteAnnouncementBtns = document.querySelectorAll('[data-action="delete-announcement"]');
        Array.prototype.forEach.call(deleteAnnouncementBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                if (confirm('Delete this announcement?')) {
                    self.data.announcements = self.data.announcements.filter(function(a) { return a.id != btn.dataset.id; });
                    self.saveData();
                    self.renderAdminDashboard();
                }
            });
        ...
        // Suggestions - Reply and Status Change
            var replySuggestionBtns = document.querySelectorAll('[data-action="reply-suggestion"]');
            Array.prototype.forEach.call(replySuggestionBtns, function(btn) {
                if (btn.classList.contains('listener-added')) return;
                btn.classList.add('listener-added');
                btn.addEventListener('click', function() {
                    var suggestionId = btn.dataset.id;
                    var replyInput = document.getElementById('reply-' + suggestionId);
                    var statusSelect = document.getElementById('status-' + suggestionId);
                    var replyContent = replyInput.value.trim();
                    if (replyContent) {
                        var suggestion = self.data.suggestions.find(function(s) { return s.id == suggestionId; });
                        if (suggestion) {
                            if (!suggestion.replies) suggestion.replies = [];
                            suggestion.replies.push({
                                content: replyContent,
                                date: new Date().toISOString().split('T')[0]
                            });
                            if (statusSelect && statusSelect.value) {
                                suggestion.status = statusSelect.value;
                            }
                            self.saveData();
                            self.renderAdminDashboard();
                        }
        ...
        // Confirm End Semester - Deactivate All Students
        ...
        if (confirmEndSemesterBtn) {
            confirmEndSemesterBtn.addEventListener('click', function() {
                if (confirm('Are you sure you want to deactivate ALL active students? This cannot be undone!')) {
                    var newBatch = document.getElementById('new-batch-year').value;
                    self.data.users.forEach(function(u) {
                        if (u.role === 'student' && u.active) {
                            u.active = false;
                            u.status = 'pending_reactivation';
                        }
                        if (u.role === 'student') {
                            u.batch = newBatch;
                        }
                    ...
                    endSemesterModal.style.display = 'none';
                    self.renderAdminDashboard();
                    alert('All students have been deactivated.');
                }
            });
        }
        ...
        // Edit Transaction
        var editTransactionBtns = document.querySelectorAll('[data-action="edit-transaction"]');
        Array.prototype.forEach.call(editTransactionBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                var transaction = self.data.finance.transactions.find(function(t) { return t.id == btn.dataset.id; });
                if (transaction) {
                    var financeModal = document.getElementById('finance-modal');
                    var financeForm = document.getElementById('finance-form');
                    financeForm.dataset.editId = transaction.id;
                    document.getElementById('finance-type').value = transaction.type || 'expense';
                    document.getElementById('finance-event').value = transaction.eventId || '';
        ...
        // Delete Transaction
        var deleteTransactionBtns = document.querySelectorAll('[data-action="delete-transaction"]');
        Array.prototype.forEach.call(deleteTransactionBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                if (confirm('Delete this transaction?')) {
                    self.data.finance.transactions = self.data.finance.transactions.filter(function(t) { return t.id != btn.dataset.id; });
                    self.saveData();
                    self.renderAdminDashboard();
                }
            });
        ...
        var viewComplaintBtns = document.querySelectorAll('[data-action="view-complaint"]');
        Array.prototype.forEach.call(viewComplaintBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                var complaint = self.data.complaints.find(function(c) { return c.id == btn.dataset.id; });
                if (complaint) {
                    var modal = document.getElementById('complaint-detail-modal');
                    var content = document.getElementById('complaint-detail-content');
                    content.innerHTML = '<p><strong>Author:</strong> ' + complaint.author + '</p>' +
                        '<p><strong>Date:</strong> ' + complaint.date + '</p>' +
                        '<p><strong>Status:</strong> ' + complaint.status + '</p>' +
                        '<p><strong>Content:</strong> ' + complaint.content + '</p>' +
                        (complaint.attachment ? '<p><strong>Attachment:</strong> <a href="' + complaint.attachment + '" target="_blank">View Attachment</a></p>' : '');
                    modal.style.display = 'flex';
                }
            });
        });
        ...
        // Chat Complaint
        var chatComplaintBtns = document.querySelectorAll('[data-action="chat-complaint"]');
        Array.prototype.forEach.call(chatComplaintBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
            btn.addEventListener('click', function() {
                var complaint = self.data.complaints.find(function(c) { return c.id == btn.dataset.id; });
                if (complaint) {
                    var modal = document.getElementById('complaint-chat-modal');
                    var chatContent = document.getElementById('complaint-chat-content');
                    var repliesHtml = complaint.replies && complaint.replies.length ? 
                        complaint.replies.map(function(r) { return '<div style="padding:8px;margin:4px;background:#f0f0f0;border-radius:4px;"><strong>' + (r.fromAdmin ? 'Admin' : complaint.author) + ':</strong> ' + r.content + '</div>'; }).join('') : 
                        '<p style="color:var(--text-light);">No replies yet.</p>';
                    chatContent.innerHTML = '<div style="max-height:300px;overflow-y:auto;margin-bottom:12px;">' + repliesHtml + '</div>' +
                        '<input type="text" class="form-input" id="complaint-reply-' + complaint.id + '" placeholder="Write reply...">' +
                        '<button class="btn btn-primary btn-sm mt-4" id="send-complaint-reply-' + complaint.id + '">Send Reply</button>';
                    modal.style.display = 'flex';
                    
                    document.getElementById('send-complaint-reply-' + complaint.id).addEventListener('click', function() {
                        var replyInput = document.getElementById('complaint-reply-' + complaint.id);
                        var replyContent = replyInput.value.trim();
                        if (replyContent) {
                            if (!complaint.replies) complaint.replies = [];
                            complaint.replies.push({ content: replyContent, fromAdmin: true, date: new Date().toISOString().split('T')[0] });
                            self.saveData();
        ...
        var changeComplaintStatusBtns = document.querySelectorAll('[data-action="change-complaint-status"]');
        Array.prototype.forEach.call(changeComplaintStatusBtns, function(btn) {
            if (btn.classList.contains('listener-added')) return;
            btn.classList.add('listener-added');
        ...
        Array.prototype.forEach.call(markAttendedBtns, function(btn) {
            ...
            btn.addEventListener('click', function() {
                ...
                if (event) {
                    ...
                    var alreadyAttended = event.attendees.some(function(a) { return a.email === self.currentUser.email; });
                    if (!alreadyAttended) {
                        ...
                        self.saveData();
                        self.renderStudentDashboard();
                        alert('You have successfully marked yourself as attended!');
                    } else {
                        alert('You have already marked yourself as attended for this event.');
                    }
                }
            });
        });

        // Student - Verify QR Code and Attend Event
        var verifyQrBtn = document.getElementById('verify-qr-btn');
        if (verifyQrBtn) {
            verifyQrBtn.addEventListener('click', function() {
                ...
                if (qrRecord) {
                    ...
                    if (qrRecord.eventId) {
                        event = self.data.events.find(function(e) { return e.id == qrRecord.eventId; });
                    }
                } else {
                    event = self.data.events.find(function(e) { return e.qrCode === code; });
                ...
                document.getElementById('scanned-event-info').innerHTML = 
                    '<p>Date: ' + (event ? event.date : new Date().toISOString().split('T')[0]) + '</p>' +
                    '<p>Time: ' + (event ? event.time : '') + '</p>' +
                    '<p>Location: ' + (event ? event.location : 'N/A') + '</p>' +
                    (qrRecord ? '<p>Code: ' + qrRecord.code + '</p>' : '');
                
                document.getElementById('attend-event-btn').dataset.qrCode = code;
                document.getElementById('attend-event-btn').dataset.eventId = event ? event.id : '';
            });
        }

        // Student - Attend Event Button
        var attendEventBtn = document.getElementById('attend-event-btn');
        if (attendEventBtn) {
            attendEventBtn.addEventListener('click', function() {
                ...
                if (qrRecord) {
                    ...
                    var alreadyAttended = qrRecord.attendees.some(function(a) { return a.email === self.currentUser.email; });
                    if (!alreadyAttended) {
                        ...
                        self.saveData();
                        alert('You have successfully attended! Thanks, ' + self.currentUser.name);
                    } else {
                    ...
                    if (event) {
                        ...
                        if (!alreadyAttended) {
                            event.attendees.push({
                                ...
                                section: self.currentUser.section || '',
                                date: new Date().toISOString().split('T')[0],
                                rsvp: event.rsvps && event.rsvps.indexOf(self.currentUser.email) > -1
                            });
                            self.saveData();
                            alert('You have successfully attended! Thanks, ' + self.currentUser.name);
                        } else {
