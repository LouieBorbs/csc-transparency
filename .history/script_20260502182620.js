    ...
    setupEventListeners: function() {
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.dropdown-toggle')) {
                ...
                var dropdown = btn.closest('.profile-dropdown') || btn.closest('.btn-group');
                if (dropdown) {
                    var menu = dropdown.querySelector('.dropdown-menu');
                    if (menu) menu.classList.toggle('show');
                }
            } else if (!e.target.closest('.profile-dropdown') && !e.target.closest('.btn-group')) {
...
renderHeader: function(user) {
    ...
    renderStudentSidebar: function() {
        ...
        // Add QR Scan modal HTML to sidebar so it is accessible from any tab (Fix 4)
